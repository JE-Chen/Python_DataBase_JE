...
# ----------------------------------------------------------------------------------------------
    ...
    def Create_Table(self,*args,Key_ID=True):
        ...
        if Key_ID:
            ...
            if len(args) == 1:
                ...
                """
                self.ValueError_Log('No enough args')
                ...
                self.ValueError_Log('Too many args')
            ...
            if len(args) == 1:
                ...
                Table_Name = args[0]
                Field = args[1]
                SQL_Command = '''CREATE TABLE IF NOT EXISTS ''' + Table_Name + '''(''' + Field + ''')'''
...
# ----------------------------------------------------------------------------------------------
    ...
    def Insert_Into(self,*args):
        ...
        if len(args) == 1:
            ...
            Table_Name = args[0]
            Value = args[1]
            SQL_Command = '''INSERT INTO ''' + Table_Name + ''' VALUES (''' + Value + ''')'''
...
# ----------------------------------------------------------------------------------------------
    ...
    def Insert_Into_Ignore(self, *args):
        ...
        if len(args) == 1:
            """
            Table_Name = args[0]
            SQL_Command = '''INSERT OR IGNORE INTO ''' + Table_Name + ''' (name) VALUES ("小明")'''
            """
            self.ValueError_Log('No enough args')
            ...
            Table_Name = args[0]
            Value = args[1]
            SQL_Command = '''INSERT OR IGNORE INTO ''' + Table_Name + ''' VALUES (''' + Value + ''')'''

        elif len(args) == 3:
            Table_Name = args[0]
            Table_Field_Name = args[1]
            Value = args[2]
            SQL_Command = '''INSERT OR IGNORE INTO ''' + Table_Name + Table_Field_Name + ''' VALUES (''' + Value + ''')'''
...
# ----------------------------------------------------------------------------------------------
    ...
    def Insert_Into_Replace(self, *args):
        ...
        if len(args) == 1:
            ...
            Table_Name = args[0]
            Value = args[1]
            SQL_Command = '''REPLACE INTO ''' + Table_Name + ''' VALUES (''' + Value + ''')'''
...
# ----------------------------------------------------------------------------------------------
    ...
    def Select_Distinct(self, *args):
        ...
        if len(args) == 1:
            ...
            Filed_Name = args[0]
            Table_Name = args[1]
            SQL_Command = '''SELECT DISTINCT ''' + Filed_Name + ''' FROM ''' + Table_Name

        else:
            self.ValueError_Log('Too many args')

        return self.Sqlite_Control.Select_Distinct(self.SQLite_Secure.Check(SQL_Command))
